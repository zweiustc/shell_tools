...
function attach() {
    ...
    iscsiadm --mode discoverydb --type sendtargets --portal ${my_ip} --discover > /dev/null
    iscsiadm --mode node --targetname ${target_name} --portal ${my_ip}:3260 --login > /dev/null
    #iscsiadm --mode discoverydb --type sendtargets --portal ${my_ip} --discover 
    #iscsiadm --mode node --targetname ${target_name} --portal ${my_ip}:3260 --login 
    ...
    #iscsiadm -m session -P 3 | grep -A 50 ${target_name} | grep "Attached scsi disk" | awk '{print $4}' > /tmp/iscsi-disk
    #device=`cat /tmp/iscsi-disk`
...
do
    ...
    attach $volumn_id
    mountpoint="/home/bcc/instances/${uuid}"
    #checkout the directory exists
    if [[ ! -d "$mountpoint" ]]; then
        echo "$mountpoint not found ... create it"
        #mkdir -p $mountpoint
    fi
    sleep 3
