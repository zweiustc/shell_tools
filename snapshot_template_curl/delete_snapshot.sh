...
if [ $# != 2 ]
then
    sh delete_snapshot.sh vm_id snapsot_id 
    echo "exit"
    exit
fi
...
#echo $server_id
url="$compute_endpoint/servers/$1/snapshots/$2"
echo $url
curl -i "$url" -X DELETE -H "Content-Type: application/json" -H "Accept: application/json" -H "X-Auth-Token:${token_id}" -d "{\"RollbackSnapshot\":null}"
