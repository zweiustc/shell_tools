...
class CHost extends CHostGeneral {
	...
	/**
	 * Create some hosts and add them to a best proxy.
	 *
	 * @param array  $options       : 
	 *	[
	 *              {
	 ...
	 *      }
	 */
	public function addHostsToProxy(array $options) {
                $counter = count($options);
                for ($i = 0; $i < $counter; $i++) {
                        $hostid = $this->createByName($options[$i]);
                        $host[$i] = $hostid['hostids'][0];
                }

                $proxyid = API::Proxy()->choiceProxy();
                if (empty($proxyid['proxyid'])) {
                        self::exception(ZBX_API_ERROR_PARAMETERS, _('No proxy has been found.'));
                }
	...
	/**
	 * Create one host by hostname.
	 *
	 * @param string $options[host]         : host you will add.                    Required
	 * @param string $options[ip]           : host ip that you will set to host.    Required
	 * @param array/string  $options[templates]    : template that you will set to host.   Required
	 * @param array/string  $options[groups]       : group where you will set host in.     Required
	 *
	 * @return array        : hostids
	 * @use case below:
	 *	Request
	 *	{
	 *              host      : "vm-10",
	 *              ip        : "10.1.128.10",
	 *              templates : ["vm template", "linux template"],
	 *              groups    : ["user1group", "user2group"]
	 *	},
	 *	Response
	 *	{
	 *		hostids   : ['10016']
	 *	}
	 */
	public function createByName(array $options) {
		$hostname = $options['host'];
		$hostip = $options['ip'];
		$templates = $options['templates'];
		$groups = $options['groups'];

		if (empty($hostname) || empty($hostip) || empty($templates)) {
			self::exception(ZBX_API_ERROR_PARAMETERS, _('Error: Host informations is incomplete.'));
		}

		$groupsId = API::HostGroup()->get(array(
				'output' => 'groupid',
				'filter' => array('name' => $groups)
				));
		if (empty($groupsId)) {
			self::exception(ZBX_API_ERROR_PARAMETERS, _('Error: the given groups cannot find.'));
		}

		$templatesId = API::Template()->get(array(
				'output' => 'templateid',
				'filter' => array('host' => $templates)
				));
		if (empty($templatesId)) {
			self::exception(ZBX_API_ERROR_PARAMETERS, _('Error: the given templates cannot find.'));
		}
	...
	/**
	 * Delete Hosts by Hostnames.
	 *
	 * @param array $hostNames
	 * @such as [
	 *		{"host": "vm192.168.227.140"},
	 *		{"host": "vm192.168.227.141"}
	 *	    ]
	 *
	 * @return array
	 * @such as [
	 ...
	 *	    ]
	 */
	public function deleteByHostName($hostNames, $nopermissions = false) {
		$hostNames = zbx_toArray($hostNames);

		// weather the HostNames existed and legal
		if (empty($hostNames)) {
		...
		if ($hostNames) {
			foreach ($hostNames as $host) {
				if (!check_db_fields(array('host' => null), $host)) {
					self::exception(ZBX_API_ERROR_PARAMETERS, _('Error: No host Name was given'));
				}
			}
			$hostNames = zbx_objectValues($hostNames, 'host');
		}
		$options = array();
		$hostIds = array();
		...
		$hostInfo = $this->get($options);
		$counter = count($hostInfo);

		// delete hosts
		if ($counter < $num) {
			self::exception(ZBX_API_ERROR_PARAMETERS, _('Error: some hosts does not exist'));
		}
		else {
			$result = $this->delete($hostInfo, $nopermissions);
		}
