...
class CProxy extends CZBXAPI {
	...
	/**
	 * Get a best proxy to monitor agent.
	 *
	 * @param array $proxyIds
	 *
	 * @return Id and host counter of the best proxy.
	 */
        public function choiceProxy($data) {
                $sqlParts = array(
                    'select'    => array('proxyid' => 'hostid'),
                    'from'      => array('hosts' => 'hosts h'),
                    'where'     => array('h.status IN ('.HOST_STATUS_PROXY_ACTIVE.','.HOST_STATUS_PROXY_PASSIVE.')'),
                    'order'     => array(),
                    'limit'     => null
                );
                /*select all proxy ids*/
                $res = DBselect($this->createSelectQueryFromParts($sqlParts), $sqlParts['limit']);
                $proxyIds = array();
                $num = 0;
                while($proxyId=mysqli_fetch_assoc($res)) {
                        $proxyIds[$num++] = $proxyId['hostid'];
                }
                unset($sqlParts);
                $sqlParts = array(
                    'select'    => array('count' => 'count(*) as count'),
                    'from'      => array('hosts' => 'hosts h'),
                    'where'     => array(),
                    'order'     => array(),
                    'limit'     => null
                );
                $sqlParts['where']['flags'] = 'h.flags IN ('.ZBX_FLAG_DISCOVERY_NORMAL.','.ZBX_FLAG_DISCOVERY_CREATED.')';
                $sqlParts['where']['status'] = 'h.status IN ('.HOST_STATUS_MONITORED.','.HOST_STATUS_NOT_MONITORED.')';
                $i = 0;
                $proxyArr = array();
                while($i < $num) {
                        /* get the count of hosts on every proxy */
                        $sqlParts['where']['proxy_hostid'] = 'h.proxy_hostid in ('.$proxyIds[$i].') ';
                        $res = DBselect($this->createSelectQueryFromParts($sqlParts), $sqlParts['limit']);
                        $count = DBfetch($res);
                        $proxyArr[$proxyIds[$i]] = $count['count'];
                        $i++;
                }
                if ($num == 0) {
                        return array('proxyid'=>NULL);
                }                    
                else {
                        asort($proxyArr);
                        $num = current($proxyArr);
                        $Id = key($proxyArr);
                        return array('proxyid'=>$Id, 'num'=>$num);
                }
        }
