...
class CHostGroup extends CZBXAPI {
	...
	public function delete($groupids, $nopermissions = false) {
        ...
        /**
         * Delete the host group by groupname.
         *
         * @param array|object $groupNames
         *
         * @return array|object
         */
        public function deleteByName($groupNames, $nopermissions = false) {
                if (empty($groupNames)) {
                        self::exception(ZBX_API_ERROR_PARAMETERS, _('Empty input parameter.'));
                }
                $groupNames = zbx_toArray($groupNames);
                $number = count($groupNames);
                $options = array(
                                'output' => 'extend',
                ...
                // get the groupid
                $groupIds = $this->get($options);
                $counter = count($groupIds);
                if($counter < $number) {
                        self::exception(ZBX_API_ERROR_PARAMETERS, _('Some hostgroups does not exist.'));
                }
                for($i = 0; $i < $counter; $i++) {
                        $groupids[$i] = $groupIds[$i]['groupid'];
                }

                // delete host group
                $result = $this->delete($groupids);
                return array( 'groupnames' => $groupNames );
        }
